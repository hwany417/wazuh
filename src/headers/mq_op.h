 ...
 *  @param n_tries Number of attempts to connect to the queue (0 to attempt until a successful conection)
 *  @return
 *  UNIX -> OS_INVALID if queue failed to start
 *  UNIX -> int(rc) file descriptor of initialized queue
 *  WIN32 -> 0
 */
int StartMQ(const char *key, short int type, short int n_tries) __attribute__((nonnull));
